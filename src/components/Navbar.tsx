      ...
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <NavigationMenu>
          <NavigationMenuList className="hidden md:flex gap-8">
            {["Services", "Testimonials", "FAQ", "Contact"].map((item) => (
              <NavigationMenuItem key={item}>
                <NavigationMenuLink
                  onClick={() => scrollToSection(item.toLowerCase())}
                  className="text-white/90 hover:text-white cursor-pointer px-4 py-2 rounded-md hover:bg-white/10 transition-colors"
                >
                  {item}
                </NavigationMenuLink>
              </NavigationMenuItem>
